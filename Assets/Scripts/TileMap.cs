...
public class TileMap {
	...
	public TileMap (int w, int h, Heightmap hmap){
	...
	void GenerateHeightMap(){
		for(int x = 0; x < sizeX; x++){
			for(int z = 0; z < sizeZ; z++){
				...
				//tiles[x,z].elevation = Mathf.Round(Mathf.PerlinNoise(seed + x/heightScaleX, seed + z/heightScaleZ) * heightStep) * heightStepInverse * heightMul;
			}
		}
	}
